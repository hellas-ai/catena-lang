...
#[test]
fn sum_bf16_exec() -> anyhow::Result<()> {
    use half::bf16;

    let runtime = runtime_with("")?;
    let values = [1.5_f32, -0.5, 2.0, 4.0].map(bf16::from_f32);
    let input = runtime.mem_bf16(&values)?;
    let [Value::BF16(result)] = runtime.exec("sum-bf16", [input.as_ref().into()])? else {
        anyhow::bail!("sum-bf16 returned the wrong value kind");
    };
    assert_eq!(result, bf16::from_f32(7.0));
    Ok(())
}

#[test]
fn mean_bf16_exec() -> anyhow::Result<()> {
    use half::bf16;

    let runtime = runtime_with("")?;
    let values = [1.5_f32, -0.5, 2.0, 4.0].map(bf16::from_f32);
    let input = runtime.mem_bf16(&values)?;
    let [Value::BF16(result)] = runtime.exec("mean-bf16", [input.as_ref().into()])? else {
        anyhow::bail!("mean-bf16 returned the wrong value kind");
    };
    assert_eq!(result, bf16::from_f32(1.75));
    Ok(())
}

#[test]
fn max_bf16_exec() -> anyhow::Result<()> {
    use half::bf16;

    let runtime = runtime_with("")?;
    let values = [1.5_f32, -0.5, 2.0, 4.0].map(bf16::from_f32);
    let input = runtime.mem_bf16(&values)?;
    let [Value::BF16(result)] = runtime.exec("max-bf16", [input.as_ref().into()])? else {
        anyhow::bail!("max-bf16 returned the wrong value kind");
    };
    assert_eq!(result, bf16::from_f32(4.0));
    Ok(())
}
