...
impl SessionOperationGate {
    ...
    fn seal_model_release_deferrals(&self, operation: ActiveSessionOperation) -> Vec<u64> {
        let mut state = self
            .state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
...
impl SessionOperationLease {
    ...
    fn seal_model_release_deferrals(&self) -> Vec<u64> {
...
fn initialize_parent_ipc<T, E>(
    ...
    match interface {
        ...
        ParentInterface::ResidentProtocol => Ok(None),
    }
}
...
impl SafeRuntime {
    ...
    fn with_interface(
            ...
            .map_err(map_init_worker_error)?
        {
            ...
            | Response::Resident(_)
            | Response::Executed(_)
            | Response::OutputsReleased => Err(SafeInitError::UnexpectedResponse),
        }
    }
        ...
        if teardown.is_ok() {
            for model in deferred_models {
                let released = worker.under_deadline(
                    ...
                    timeout,
                    DeadlineKind::Execution,
                    |worker, deadline| worker.request(&Request::ReleaseModel { model }, deadline),
                );
                teardown = match released {
                    Ok(Response::Resident(Ok(ResidentResponse::ModelReleased(id))))
                        if id == model =>
                    {
                        Ok(())
                    }
    ...
    pub(crate) fn release_resident_model(&self, model: ResidentModel) -> Result<(), ResidentError> {
        let Some(_operation) = self
            .operation_gate
            .acquire_or_defer_model_release(model.id)
        ...
        let response = match worker.with_deadline(
            self.timeouts.execution_timeout(),
            DeadlineKind::Execution,
            |worker, deadline| worker.request(&Request::ReleaseModel { model: model.id }, deadline),
        ) {
        ...
        match response {
            Response::Resident(Ok(ResidentResponse::ModelReleased(id))) if id == model.id => Ok(()),
    ...
    pub fn exec<'a, const M: usize, const N: usize>(
        ...
        let values = self.exec_values(artifact, name, args.into())?;
...
fn execution_from_response(
    ...
    match response {
        ...
        | Response::Resident(_)
        | Response::OutputsReleased => {
            Err(worker.reject_protocol_response(SafeExecError::UnexpectedResponse))
        }
    }
}
...
mod error_tests {
    ...
    fn structured_rejections_preserve_the_session() {
        ...
        assert!(!SafeExecError::Runtime(ExecError::UnknownArtifact).invalidates_session());
    ...
    fn same_thread_generation_reentry_is_rejected_and_model_drop_is_deferred() {
        ...
        assert_eq!(generation.seal_model_release_deferrals(), vec![17, 23]);
        drop(generation);
    ...
    fn preopen_model_release_defers_after_start_and_waits_after_seal() {
        ...
        thread::scope(|scope| {
            ...
            assert_eq!(generation.seal_model_release_deferrals(), vec![17]);
